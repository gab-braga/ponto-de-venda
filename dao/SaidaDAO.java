...
public abstract class SaidaDAO {
        ...
        if (ConnectionFactory.createDatabase()) {
            if (CaixaDAO.createTable() && UsuarioDAO.createTable()) {
                if (ConnectionFactory.openConnection()) {
                    try {
                        String sql =
                                "CREATE TABLE IF NOT EXISTS " + ConnectionFactory.database + ".saida(" +
    ...
    private static List<Saida> getExitsList(ResultSet results) throws SQLException {
        ...
        while (results.next()) {
            ...
            Usuario operator = UsuarioDAO.queryUserByName(results.getString("USUARIO_NOME")).get(0);
            saidas.add(new Saida(code, value, Datetime.getTimestamp(date, hour), reason, caixa, operator));
        }
        return saidas;
    }
    ...
    public static boolean register(Saida saida) {
        ...
        if(createTable()) {
            if (ConnectionFactory.openConnection()) {
                try {
                    String sql = "INSERT INTO " + ConnectionFactory.database + ".saida (SAIDA_VALOR, SAIDA_DATA, SAIDA_HORA, SAIDA_MOTIVO, CAIXA_CODIGO, USUARIO_NOME) VALUES (?, ?, ?, ?, ?, ?);";
    ...
    public static List<Saida> queryAllExits() {
        ...
        if(createTable()) {
            if (ConnectionFactory.openConnection()) {
                try {
                    String sql = "SELECT * FROM " + ConnectionFactory.database + ".saida ORDER BY SAIDA_DATA;";
                    PreparedStatement statement = ConnectionFactory.connection.prepareStatement(sql);
    ...
    public static List<Saida> queryExitsByDate(java.util.Date date) {
        ...
        if(createTable()) {
            if (ConnectionFactory.openConnection()) {
                try {
                    String sql = "SELECT * FROM " + ConnectionFactory.database + ".saida WHERE SAIDA_DATA = ? ORDER BY SAIDA_DATA;";
                    PreparedStatement statement = ConnectionFactory.connection.prepareStatement(sql);

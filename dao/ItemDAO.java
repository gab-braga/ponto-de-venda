...
public abstract class ItemDAO {
        ...
        if (ConnectionFactory.createDatabase()) {
            if (ProdutoDAO.createTable() && VendaDAO.createTable()) {
                if (ConnectionFactory.openConnection()) {
                    try {
                        String sql =
                                "CREATE TABLE IF NOT EXISTS " + ConnectionFactory.database + ".item(" +
    ...
    private static List<Item> getItemsList(ResultSet results) throws SQLException {
        ...
        while (results.next()) {
            ...
            items.add(new Item(code, codeProduct, codeSale, quantity));
        }
        return items;
    }
    ...
    public static boolean register(Item item) {
        ...
        if(createTable()) {
            if (ConnectionFactory.openConnection()) {
                try {
                    String sql = "INSERT INTO " + ConnectionFactory.database + ".item (ITEM_QUANTIDADE, PRODUTO_CODIGO, VENDA_CODIGO) VALUES (?, ?, ?);";
                    PreparedStatement statement = ConnectionFactory.connection.prepareStatement(sql);
                    statement.setInt(1, item.getQuantidade());
                    statement.setInt(2, item.getCodigoProduto());
                    statement.setInt(3, item.getCodigoVenda());
    ...
    public static List<Item> queryAllItems() {
        ...
        if(createTable()) {
            if (ConnectionFactory.openConnection()) {
                try {
                    String sql = "SELECT * FROM " + ConnectionFactory.database + ".item ORDER BY ITEM_CODIGO;";
                    PreparedStatement statement = ConnectionFactory.connection.prepareStatement(sql);
    ...
    public static List<Item> queryItemsByCodeProductOrCodeSale(Item item) {
        ...
        if(createTable()) {
            if (ConnectionFactory.openConnection()) {
                try {
                    String sql = "SELECT * FROM " + ConnectionFactory.database + ".item WHERE PRODUTO_CODIGO = ? OR VENDA_CODIGO = ? ORDER BY ITEM_CODIGO;";
                    PreparedStatement statement = ConnectionFactory.connection.prepareStatement(sql);
                    statement.setInt(1, item.getCodigoProduto());
                    statement.setInt(2, item.getCodigoVenda());
                    items = getItemsList(statement.executeQuery());
                } catch (SQLException e) {
